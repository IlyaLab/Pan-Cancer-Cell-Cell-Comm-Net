...
# load the c2cWts
  ...
  selFeat <- colnames(c2cWtsT)[featCor < 0.0001]
  
  dat <- c2cWtsT[,selFeat]  
  
  superheat(X = dat, yr = ifelse(clinfeature == 2, yes=1, no=0),
...
for (idx in 1:nrow(dat)) {
    ...
    modJ <- randomForest(x = as.matrix(train), y = as.factor(trainlabel), importance = T, ntree=3333)
    modelList[[idx]] <- modJ
...
write.table(featuredf, file="bailey_4_top_features_acc.csv", sep=',', quote=F, row.names = F)

featuredf <- data.frame(Gini=names(gini_features), Count=gini_features)
write.table(featuredf, file="bailey_4_top_features_gini.csv", sep=',', quote=F, row.names = F)


x <- data.frame(tcga$mRNABaileyCluster,  dat[,'Macrophages.M2_TGFB1_TGFBR2_T.cells.follicular.helper'])
x$C2 <- ifelse(x$tcga.mRNABaileyCluster == 2, yes=1, no=0)
colnames(x) <- c('Bailey', 'EdgeWts', 'C2indicator')
qplot(data=x, y=EdgeWts, x=as.factor(C2indicator), geom="boxplot") + theme_bw() + xlab("C2, immunogenic cluster, indicator")
