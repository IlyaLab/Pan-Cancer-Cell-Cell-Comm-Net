...
# Cell maps
celltype_map <- read_tsv("Data/new_celltype_map_march8.tsv")
...
qplot(x=log(as.numeric(dataList[[3]]$`tpm.Adipocyte%20-%20breast%2c%20donor1.CNhs11051.11376-118A8`)), geom='density') + geom_histogram()

X <- (as.numeric(dataList[[3]]$`tpm.Adipocyte%20-%20breast%2c%20donor1.CNhs11051.11376-118A8`))
quantile(X[X > 0])
...
# fix nov col names
novCols <- colnames(dataList[[7]])
novCols2 <- str_split(novCols, ',')
novCols3 <- lapply(novCols2, function(a) a[1])
novCols <- unlist(novCols3)
colnames(dataList[[7]]) <- novCols
rm(novCols2, novCols3)

colnames(dataList[['encode']])[2] <- 'GeneSymbol'
colnames(dataList[[3]])[1] <- 'GeneSymbol'
...
save(scaffold, file='scaffold_median_cut_march11.rda')
