...
for (idx in 1:nrow(dat)) {
    ...
    modJ <- randomForest(x = as.matrix(train), y = as.factor(trainlabel), importance = T, mtry = 18, ntree=2000)
    modelList[[idx]] <- modJ
...
predrand <- prediction(res0$Raw, sample(x = c(1,0), size = length(res0$Raw), replace = T))
...
write.table(featuredf, file="os_top_features_acc.csv", sep=',', quote=F, row.names = F)

featuredf <- data.frame(Gini=names(gini_features), Count=gini_features)
write.table(featuredf, file="os_top_features_gini.csv", sep=',', quote=F, row.names = F)


x <- data.frame(PFI=resp$OS,  EdgeWts=dat[,'Mast.cells.activated_CCL2_CCR4_T.cells.regulatory..Tregs.'])
qplot(data=x, y=EdgeWts, x=as.factor(PFI), geom="boxplot") + theme_bw() + xlab("OS")
