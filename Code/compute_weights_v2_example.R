...
# paths relative to root Pan-Cancer-Cell-Cell-Comm-Net dir.
...
workFun <- function(si, filepath) {
  ...
  ccs <- read.csv('Example/small_scaffold.csv', header=T, stringsAsFactors=F)
  ...
  for (ei in 1:nrow(ccs)) {
	...
	try ({
        ...
        if (curr1 != paste0(c1,bin1)) {
          # then load it.
          load(paste0('Distributions/GeneProbDist/prob_',c1,'_',bin1,'.rda'))
          p1 <- x
        ...
        if (curr2 != paste0(c2,bin2)) {
          # then load it.
          load(paste0('Distributions/GeneProbDist/prob_',c2,'_',bin2,'.rda'))
          p2 <- x
...
fileout <- paste0("Example/Results/res_",ti,".rda")
