...
def makeScores(n1, n2, reps, vals):
    scores = []
    med_diff = []
    n = len(vals)
    s = set(range(0,n))
...
def main():
    ...
    for li in lines:
        bits = li.strip().split(',')
        filename = 'permSamples3/samples_'+bits[0]+'.csv'
        vals = samplevalues(filename)
        n1 = float(bits[1])
        n2 = float(bits[2])
        print(filename + '  ' + str(n1) + '   ' + str(n2))
        (med_diffs, scores) = makeScores(n1, n2, 1000000, vals)
        for i in range(0,len(scores)):
            fout.write(bits[0] + ',' + bits[1] + ',' + bits[2] + ',' + str(scores[i]) + ',' + str(med_diffs[i]) + '\n')
